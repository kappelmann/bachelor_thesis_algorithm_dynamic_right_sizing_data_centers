...
\begin{equation}
	\costs(P)=\costs(0,x_1,\lambda_1)+\sum\limits_{t=2}^{T}\costs(x_{t-1},x_{t},\lambda_{t})+\underbrace{\costs(x_T,0,0)}_{=0}=\costs(0,x_1,\lambda_1)+\sum\limits_{t=2}^{T}\costs(x_{t-1},x_{t},\lambda_{t})\label{eq:v0_VT1_path_costs}
\end{equation}
Note that the cost of such a path directly correspond to those of a schedule $\mx$ (see~\eqref{eq:mx_schedule_total_costs}).
...
\textproc{shortest\_paths} layerwise calculates the minimum costs of the nodes of our graph; that is, it calculates the shortest paths following the topological sorting of the graph. It returns the minimum costs to all nodes as well as the predecessor of any node in respect to its shortest path.
...
\begin{algorithm}[H]
  ...
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f$}
	\LineCommentFunc{Nodes' costs and predecessors tables}
	\Blet{$C[0\ldots m,0\ldots T+1]$ and $P[0\ldots m,0\ldots T+1]$}{new tables}
	\Let{$C[0,0]$}{$P[0,0]\gets 0$}\Comment{Initialize first node}
	\For{$x \gets 0 \textrm{ to } m$}\Comment{Initialize first layer}
		\Let{$C[x,1]$}{$\costs(0,x,\lambda_1)$}
		\Let{$P[x,1]$}{$0$}
	\EndFor
	\For{$t \gets 2 \textrm{ to } T$}\Comment{Iterative calculate costs and predecessors}
		...
		\EndFor
	\EndFor
	\Let{$P[0,T+1]$}{$\argmin\limits_{x\in\fromto{0}{m}}\bigl\{C[x,T]\bigr\}$}\Comment{Find best choice for last time slot}
	\Let{$C[0,T+1]$}{$C\bigl[P[0,T+1],T\bigr]$}
	\State \Return{$(C,P)$}
  \EndFunction
  \Statex
  \Function{extract\_schedule}{$P,T$}
	\Blet{$\mx[T]$}{a new array}\Comment{Allocate the schedule array}
    	\Let{$\mx[T]$}{$P[0,T+1]$}\Comment{Get best choice for last time slot}
...
Subroutine \textproc{shortest\_paths} needs $\Theta(m)$ iterations for its initialization, $\Theta(Tm^2)$ steps for the iterative calculation, and $\Theta(m)$ steps for its final minimization search. In addition, \textproc{extract\_schedule} needs $\Theta(T)$ iterations for its schedule retrieval. Thus, we receive a runtime of 
\begin{equation*}
	\Theta(m+Tm^2+m+T)=\Theta(Tm^2)
\end{equation*}
The runtime is polynomial in the numeric value of $m$ and $T$; however, it is exponential in the size of the input because, as we saw in~\eqref{eq:inp_size}, we only need $\log_2(m)$ bits to encode $m$. Hence, the algorithm is pseudo-polynomial.
...
\begin{exmpl}
Let $\inp_i=(m=1,T=2,\Lambda_i,\beta=1,f)$ be the inputs for two problem instances with $f(x)=x^2+1$, $\Lambda_1=(1,0)$, and $\Lambda_2=(0,1)$. Below we illustrate the graphs for the two problem instances and the corresponding calculations done by the algorithm derived in Section~\ref{sec:opt_offline_pseudo_poly}.
\begin{figure}[H]
...
\end{equation*}
A schedule corresponding to $P$ would use one active server in its first time slot, then power down this server, and subsequently turn on two servers to process the next arrival rate. This seems somewhat unreasonable. We could just keep the initial server on to save switching costs (note the correspondence to Proposition~\ref{prop:reasonable_switching}). In fact, this behaviour cannot even be represented using our schedule notation $\mx$ and optimisation condition~\eqref{eq:mx_schedule_total_costs}. We can, however, modify $P$ to represent a more reasonable sequence, that is we set
\begin{equation*}
...
Let $t\in[T-1]$ be the first time slot falsifying~\eqref{eq:reasn_path}. If there does not exist such a time slot, we are done. Otherwise, let $E_\downarrow^t$ and $E_\uparrow^t$ be its sets of edges as defined in Definition~\ref{defn_reasn_paths}. Since $P$ falsifies~\eqref{eq:reasn_path} at time $t$, both $E_\downarrow^t$ as well as $E_\uparrow^t$ must be non-empty. Thus, we can obtain the ``maximum'' nodes involved in these sets.
\begin{align*}
...
Naturally, a most cost-efficient path $S'$ from node $v_{x_s,t\downarrow}$ to $v_{x_e,t\uparrow}$ minimizes the number of edges $(v_{x,t\uparrow},v_{x+1,t\uparrow})$ since each of these edges incurs cost $\beta\ge 0$. For this, we observe that we must not shut down servers if $x_s\le x_e$, and that we must not power on servers if $x_s\ge x_e$; thus, we consider three cases for $S'$:
\begin{equation*}
	S'\coloneqq
	\begin{cases}
...
\begin{lem}\label{lem:sched_path_pseudo_lin}
...
\end{thm} 
\begin{proof}
By Lemma~\ref{lem:sched_path_pseudo_lin}, we have a bijection $\Phi$ between reasonable paths $P$ and schedules $\mx$ obeying $\costs(P)=\costs\bigl(\Phi(P)\bigr)$. Thus, we have 
\begin{equation*}
...
\textproc{shortest\_paths} calculates the minimum costs of the nodes of our graph by following the topological sorting of the graph. It returns the minimum costs to all nodes as well as the predecessor of any node in respect to its shortest path.
...
\begin{algorithm}[H]
  ...
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f$}
	\LineCommentFunc{Nodes' costs and predecessors tables}
	\Blet{$C[0\ldots m,0\ldots2T-1]$ and $P[0\ldots m,0\ldots2T-1]$}{new tables}
	\Let{$C[0,0]$}{$P[0,0]\gets 0$}\Comment{Initialize first node}
	\Let{$C[m,1]$}{$\costs(0,m,\lambda_1)$ and $P[m,1]\gets 0$}\Comment{Initialize first layer}
	\For{$x \gets m-1 \textrm{ to } 0$}
		\Let{$C[x,1]$}{$\costs(0,x,\lambda_1)$ and $P[x,1]\gets 0$}
	\EndFor
	\For{$t \gets 1 \textrm{ to } T-1$}\Comment{Iterative calculate costs and predecessors}
		\For{$x \gets m-1 \textrm{ to } 0$}
			\If {$C[x+1,2t-1]<C[x,2t-1]$}
			    \Let{$C[x,2t-1]$}{$C[x+1,2t-1]$ and $P[x,2t-1]\gets P[x+1,2t-1]$}
			\EndIf
		\EndFor
		\Let{$C[0,2t]$}{$C[0,2t-1]$ and $P[0,2t]\gets P[0,2t-1]$}
		\For{$x \gets 1 \textrm{ to } m$}
			\If {$C[x-1,2t]<C[x,2t-1]$}
			    \Let{$C[x,2t]$}{$C[x-1,2t]$ and $P[x,2t]\gets P[x-1,2t]$}
			\Else
			    \Let{$C[x,2t]$}{$C[x,2t-1]$ and $P[x,2t]\gets P[x,2t-1]$}
			\EndIf
		    	\Let{$C[x,t+1]$}{$C[x,t]+\opcosts(x,\lambda_x)$ and $P[x,t+1]\gets P[x,t]$}
		\EndFor
	\EndFor
