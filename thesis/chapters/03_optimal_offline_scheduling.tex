...
\begin{algorithm}[H]
  ...
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f$}
	\LineComment{Nodes' costs and predecessors tables}
	\Blet{$C[0\ldots m,0\ldots T+1]$ and $P[0\ldots m,0\ldots T+1]$}{new tables}
	\Let{$C[0,0]$}{$P[0,0]\gets 0$}\Comment{Initialization for first node}
	\For{$x \gets 0 \textrm{ to } m$}\Comment{Initialization for first layer}
...
\begin{algorithm}[H]
  ...
  \Statex
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f$}
	\Blet{$C[2T,m+1]$ and $P[2T,m+1]$}{new arrays}\Comment{Nodes' costs and predecessors}
	\Let{$C[0,0]$}{$P[0,0]\gets 0$}
	\For{$x \gets 0 \textrm{ to } m$}\Comment{Initialization steps}
		\Let{$C[1,x]$}{$\costs(0,x,\lambda_1)$}
		\Let{$P[1,x]$}{$0$}
	\EndFor
	\For{$t \gets 1 \textrm{ to } T-1$}\Comment{Iterative calculation of costs and predecessors}
		\For{$x \gets m-1 \textrm{ to } 0$}
			\If {$C[x+1,2t-1]<C[x,2t-1]$}
			    \Let{$C[x,2t-1]$}{$C[x+1,2t-1]$}
			    \Let{$P[x,2t-1]$}{$P[x+1,2t-1]$}
			\EndIf
		\EndFor
		\Let{$C[0,2t]$}{$C[0,2t-1]$}
		\Let{$P[0,2t]$}{$P[0,2t-1]$}
		\For{$x \gets 1 \textrm{ to } m$}
			...
			\Else
			    ...
			    \Let{$P[x,t]$}{$P[x,t-1]$}
			\EndIf
		    	\Let{$C[x,t+1]$}{$C[x,t]+\opcosts(x,\lambda_x)$}
			\Let{$P[x,t+1]$}{$P[x,t]$}
		\EndFor
	\EndFor
