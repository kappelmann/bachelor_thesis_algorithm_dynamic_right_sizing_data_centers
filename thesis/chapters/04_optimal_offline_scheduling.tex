...
\begin{algorithm}[H]
  ...
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f$}
	...
	\Statex \Comment{Allocate nodes' costs and predecessor tables}
...
\end{align}
For our runtime analysis, we assume that calling the operating costs function $f(\cdot)$, and as a consequence also $\costs(\cdot,\cdot,\cdot)$, incurs constant cost. Subroutine \textproc{shortest\_paths} needs $\Theta(m)$ iterations for its initialization and $\Theta(Tm^2)$ steps for the iterative calculation of predecessors and costs. In addition, \textproc{extract\_schedule} needs $\Theta(m)$ steps for its initial minimization search and $\Theta(T)$ iterations for its schedule retrieval. Thus, we receive a runtime of 
\begin{equation*}
...
\begin{equation}
	\forall t\in[T-1]:E_\downarrow^t=\emptyset \lor E_\uparrow^t=\emptyset\label{eq:reasn_path}
\end{equation}
\end{defn}
