...
\end{flalign*}
Note that $u,u'\in B$ and that $x_t\le u\le 2x_t$ holds for any $i\le t\le j$ since $\mx$ does not change its 2-state between $i$ and $j$. More precisely, we know that $x_t\in[l,u)$ if $u=2l$, and $x_t\in[l,u]$ if $u=m<2l$, for $i\le t\le j$. To combine both cases, given two boundaries $a,b\in\mathbb{N}$, we define a conditional interval:\unsure{TODO okay?}
\begin{equation*}
