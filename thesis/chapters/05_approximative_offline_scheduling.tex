...
\end{enumerate}
To finish the proof, we show that any case can be transformed to a 2-approximative period in $\mx'$ while ensuring that our invariant will not be violated. Additionally, we have to verify that the credit of our amortized analysis stays non-negative. Note that we already showed that these claims holds for our initial start-up process.
...
\begin{align*}
	...
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}y\opcosts\bigl(\mx_{i,j}\bigr)
\end{align*}
shows that $\mx_{i,j}^{\ceil{u}}$ is $y$-approximative. Next, if $y\ceil{l}\le\ceil{u}$, we choose strategy $\mx_{i,j}^{\floor{u}}$, whose cost are also $y$-approximative, as we have already seen in the previous case.
...
\end{enumerate}
All three possibilities are illustrated in Figure~\ref{fig:schedule_behavior_down_up_y}. For each possibility, we have to show that at least one of our three strategies is fitting, i.e.\ incurs $y$-approximative costs including the cost to satisfy the invariant at time $j+1$ and the credit $\beta(yx_{j+1}-\floor{u''})$ for the next step.
\begin{figure}[ht]
...
\end{figure}
We begin with case~\ref{itm:schedule_behavior_down_up_y_subcase_1}. Since $\mx$ ascends to $[u,\hat{u})$ at time~$h$ and descends to $[l,u)$ at time~$i$, we know from case~\ref{itm:schedule_behavior_up_down_y} that we have some remaining credit $\beta(x_h-\floor{\hat{u}})$.
