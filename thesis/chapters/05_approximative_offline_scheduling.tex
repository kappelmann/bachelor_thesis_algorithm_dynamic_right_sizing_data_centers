...
\end{enumerate}
To finish the proof, we need to show that any case can be transformed to a 2-approximative period in $\mx'$ while ensuring that our invariant will hold at the beginning of the next period, i.e.\ that we can set $x_{j+1}\coloneqq u'$ with 2-approximative switching costs. Additionally, we have to verify that the credit of our amortized analysis stays non-negative. In particular, we will show that our credit will be greater than or equal to $\beta(2x_{j+1}-u')$ if $\mx$ exits~$[l,u)$ by ascending to $[l',u')$ at time $j+1$ and $u'\neq m$. Since the periods are consecutive, this equivalently means that the credit will be greater than or equal to $\beta(2x_i-u)$ if $\mx$ enters~$[l,u)$ at time~$i$ from below and $u\neq m$. Note that we already showed that this claim holds for our initial start-up process.
...
\end{figure}
Due to our invariant and the fact that $\mx$ descends at time $i$, we know that $x_{i-1}'\ge \hat{u}\ge u$, and thus both strategies do no incur switching costs up to time $j$, since we can simply move down from $x_{i-1}'$ to $\hat{u}$ or $u$.
...
\begin{align*}
	\opcosts\bigl(\mx_{i,j}\bigr)+\beta(u-d)&>\opcosts\bigl(\mx_{i,j}\bigr)+\frac{2\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^u\bigr)}{\hat{u}-3u+2d}(u-d)\\
...
\begin{align*}
	2\left(\opcosts\bigl(\mx_{i,j}\bigr)+\beta(u-d)\right)-\opcosts\bigl(\mx_{i,j}^{\hat{u}}\bigr)>2\,\frac{(\hat{u}-u)\opcosts\bigl(\mx_{i,j}\bigr)-(u-d)\opcosts\bigl(\mx_{i,j}^u\bigr)}{\hat{u}-3u+2d}-\opcosts\bigl(\mx_{i,j}^{\hat{u}}\bigr)
\end{align*}
...
TODO text
Let $\beps\in\mathbb{R}_{>0}$.
\begin{align*}
	y&\coloneqq1+\beps\\
	b&\coloneqq\floor{\log_y(m)}\\
	B&\coloneqq\left\{0,\floor{y^0},\ceil{y^0},\floor{y^1},\ceil{y^1},\dotsc,\floor{y^b},\ceil{y^b},m\right\}
\end{align*}
...
\end{flalign*}
In the proof of Lemma~\ref{lem:transform_schedule_approx_2}, we knew that, for example, $u$ is contained in $B$, and thus we were able to simply use $u$ servers for $\mx'$. However, as $y$ is not necessarily integer anymore, we have to appropriately round our choices, e.g.\ use $\floor{u}\in B$ or $\ceil{u}\in B$ instead of $u$ servers.  
...
\begin{align*}
	...
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}y\opcosts\bigl(\mx_{i,j}\bigr)
\end{align*}
which shows that $\mx_{i,j}^{\ceil{u}}$ is $y$-approximative. Next, if $y\ceil{u}\le\ceil{u}$, we choose strategy $\mx_{i,j}^{\floor{u}}$, whose cost are also $y$-approximative, as we have already seen in the previous case.
...
\end{flalign*}
with $\hat{u}\coloneqq\min\{yu,m\}$. Notice that due to our invariant and the fact that $\mx$ descends at time $i$, strategies $\mx_{i,j}^{\floor{u}}$ and $\mx_{i,j}^{\floor{\hat{u}}}$ do no incur switching costs up to time $j$, since $x_{i-1}'\ge\floor{\hat{u}}\ge\floor{u}$. We are now going to prove that the cost of at least one of the three schedules, including possible switching costs to satisfy our invariant at time $j+1$, must be $y$-approximative. 
...
\begin{equation*}
	\sum\limits_{t=i+1}^{j+1}\swcosts(x_{t-1},x_t)\ge\beta(x_{j+1}-d)
\end{equation*}
Next, we have to conduct another case distinction; namely, either $yd\le\floor{\hat{u}}$ or $yd>\floor{\hat{u}}$. We begin with the former, for which we proceed analogous as in the proof of Lemma~\ref{lem:transform_schedule_approx_2}. We again split $\mx_{i,j}$ and its associated switching costs at the final switching step into two parts. This time, we split $\beta(x_{j+1}-d)$ into $\beta(x_{j+1}-\floor{\hat{u}}/y)$ and $\beta(\hat{u}/y-d)$. It now suffices to show that either $\mx_{i,j}^{\floor{u}}$ or $\mx_{i,j}^{\floor{\hat{u}}}$ -- strategy $\mx_{i,j}^{\ceil{\hat{u}}}$ will not be needed in this case -- can process the loads $\lambda_i,\dotsc,\lambda_j$ and eventually switch to $\floor{\hat{u}}$ with $y$-approximative costs under the assumption that $\mx_{i,j}$ only moves to~$\floor{\hat{u}}/y$ as a first step. 
This is because the remaining switching costs to ascend from $\floor{\hat{u}}$ to $\floor{u'}$ are then $y$-approximative anyway:
\begin{equation*}
...
\begin{equation*}
	\beta(yx_{j+1}-\floor{\hat{u}})-\beta(\floor{u'}-\floor{\hat{u}})=\beta(yx_{j+1}-\floor{u'})
\end{equation*}
...
\end{align*}
where the last step is justified by assumption $yd>\floor{u}$. Now, using our other assumption $yd\le\floor{\hat{u}}$, which equivalently means $\frac{\floor{\hat{u}}}{y}-d\ge0$, we can find a lower bound for the cost of~$\mx_{i,j}$:
\begin{equation*}
	\opcosts\bigl(\mx_{i,j})+\beta\biggl(\underbrace{\frac{\floor{\hat{u}}}{y}-d}_{\ge 0}\biggr)>\opcosts\bigl(\mx_{i,j}\bigr)+\frac{y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\floor{u}}\bigr)}{yd-\floor{u}}\left(\frac{\floor{\hat{u}}}{y}-d\right)
\end{equation*}
...
\end{align*}
Recall that $\mx_{i,j}^{\floor{\hat{u}}}$ does not incur switching costs to move to $\floor{\hat{u}}$. We can now use our just derived lower bound for $\opcosts\bigl(\mx_{i,j}\bigr)+\beta(\floor{\hat{u}}/y-d)$ to estimate the difference:
\begin{align*}
	&y\left(\opcosts\bigl(\mx_{i,j}\bigr)+\beta\left(\frac{\floor{\hat{u}}}{y}-d\right)\right)-\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)\\
	>\quad&y\, \frac{(\floor{\hat{u}}-\floor{u})\opcosts\bigl(\mx_{i,j}\bigr)-\left(\frac{\floor{\hat{u}}}{y}-d\right)\opcosts\bigl(\mx_{i,j}^{\floor{u}}\bigr)}{yd-\floor{u}}-\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)
\end{align*}
Thus, it suffices to show that
\begin{align*}
...
\end{align*}
which finishes the case with assumption $yd\le\floor{\hat{u}}$. We are left with case $yd>\floor{\hat{u}}$. For this case, we have to analyze the behavior of $\mx$ more precisely. There are three possible behaviors of $\mx$ which can lead to case~\ref{itm:schedule_behavior_down_up_y}. 
\begin{enumerate}[label=(\roman*)]
	\item $\mx$ ascends to $[u,\hat{u})$ and then descends to $[l,u)$ at time $i$.
	\item $\mx$ descends to $[u,\hat{u})$ and then descends to $[l,u)$ at time $i$.
	\item $\mx$ descends to $[l,u)$ at time $i$ from a region above $[u,\hat{u})$.
\end{enumerate}
All three possibilities are illustrated in Figure~\ref{fig:schedule_behavior_down_up_y}.
\begin{figure}[H]
...
TODO
\begin{align*}
	y\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}\bigr)+\beta(x_{j+1}-d)}^{\text{costs of }\mx_{i,j}}\bigr)&-\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)+\beta(\floor{u'}-\floor{\hat{u}})}^{\text{costs of }\mx_{i,j}^{\floor{\hat{u}}}}\bigr)-\overbrace{\beta(yx_{j+1}-\floor{u'})}^{\text{credit for next step}}\\
	&+\underbrace{\beta(y\ceil{u}-\floor{\hat{u}})}_{\text{credit from previous step}}\ge 0
\end{align*}
blub
\begin{align*}
	&&&y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)+\beta\bigl(yx_{j+1}-yd-\floor{u'}+\floor{\hat{u}}-yx_{j+1}+\floor{u'}+y\ceil{u}-\floor{\hat{u}}\bigr)\\
	&=&&y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)+\beta\bigl(y\ceil{u}-yd\bigr)\\
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}&&\sum\limits_{t=i}^{j}\left(yx_tf\left(\frac{\lambda_t}{x_t}\right)-\floor{\hat{u}}f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\right)+\beta\bigl(y\ceil{u}-yd\bigr)
\end{align*}
what
\begin{align*}
	\sum\limits_{t=i}^{j}\left(yx_tf\left(\frac{\lambda_t}{x_t}\right)-\floor{\hat{u}}f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\right)&\ge\sum\limits_{t=i}^{j}\left(ydf\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)-\floor{\hat{u}}f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\right)\\
	&=\sum\limits_{t=i}^{j}\biggl((\underbrace{yd-\floor{\hat{u}}}_{>0})f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\biggr)
\end{align*}
BLub next
assume
\begin{align*}
	y\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}\bigr)+\beta(x_{j+1}-d)}^{\text{costs of }\mx_{i,j}}\bigr)&-\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}^{\floor{\hat{u}}}\bigr)+\beta(\floor{u'}-\floor{\hat{u}})}^{\text{costs of }\mx_{i,j}^{\floor{\hat{u}}}}\bigr)-\overbrace{\beta(yx_{j+1}-\floor{u'})}^{\text{credit for next step}}<0
\end{align*}
rearranging
\begin{align*}
...
lhs
\begin{align}
	\sum\limits_{t=i}^{j}\biggl((yd-\floor{\hat{u}})f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\biggr)<\beta\bigl(yd-\floor{\hat{u}}\bigr)&&\stackrel{yd>\floor{\hat{u}}}{\iff}&&\sum\limits_{t=i}^{j}f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)<\beta
\end{align}
then next
\begin{equation*}
	y\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}\bigr)+\beta(x_{j+1}-d)}^{\text{costs of }\mx_{i,j}}\bigr)-\bigl(\overbrace{\opcosts\bigl(\mx_{i,j}^{\ceil{\hat{u}}}\bigr)+\beta(\floor{u'}-\ceil{\hat{u}})}^{\text{costs of }\mx_{i,j}^{\ceil{\hat{u}}}}\bigr)-\overbrace{\beta(yx_{j+1}-\floor{u'})}^{\text{credit for next step}}\ge0
\end{equation*}
dkd
\begin{align*}
	&y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\ceil{\hat{u}}}\bigr)+\beta(yx_{j+1}-yd-\floor{u'}+\ceil{\hat{u}}-yx_{j+1}+\floor{u'})\\
	=\quad&y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\ceil{\hat{u}}}\bigr)+\beta(\ceil{\hat{u}}-yd)
\end{align*}
daljl
\begin{align*}
	&y\opcosts\bigl(\mx_{i,j}\bigr)-\opcosts\bigl(\mx_{i,j}^{\ceil{\hat{u}}}\bigr)+\beta(\ceil{\hat{u}}-yd)\\
	...
	=&\sum\limits_{t=i}^{j}\left((yd-\floor{\hat{u}})f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\right)+\beta(\ceil{\hat{u}}-yd)\\
	=&\sum\limits_{t=i}^{j}\left(-(\floor{\hat{u}}-yd)f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)\right)+\beta(\ceil{\hat{u}}-yd)\\
	=&\Biggl(\underbrace{\beta-\sum\limits_{t=i}^{j}f\left(\frac{\lambda_t}{\floor{\hat{u}}}\right)}_{>0}\Biggr)(\underbrace{\floor{\hat{u}}-yd}_{>0})>0
\end{align*}
