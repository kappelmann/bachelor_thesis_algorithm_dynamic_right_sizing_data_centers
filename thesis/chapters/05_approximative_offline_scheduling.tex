...
As a final step of this thesis, we will generalize the 2-optimal algorithm to derive an $(1+2\beps)$-optimal algorithm with TODO time complexity.
...
\section{A $(1+2\beps)$-Optimal Linear-Time Algorithm (Integer Failed)}
TODO text
\begin{align*}
	y&\coloneqq1+\beps\\
...
\begin{align*}
	...
	G&\coloneqq(V,E,c_G)
\end{align*}
Unnecessary loops can again simply be ignored for our following work.
...
\end{defn}
TODO text
\begin{flalign}\label{eq:conditions_z_o}
	&&zo=y^{i+1}&&\text{and}&&z(y^i-o)=y^{i+1}-y^i&&&
\end{flalign}
Rearranging the first equality gives us $o=y^{i+1}/z$, which we can use to substitute in $o$ in our second equality
\begin{flalign*}
	&&&&&&z(y^i-y^{i+1}/z)&= y^{i+1}-y^i&&&&&&\\
	&&&&\iff&& zy^i-y^{i+1}&= y^i(y-1)&&&&&&&\\
	&&&&\iff&& z-y&= y-1&&&&&&&\\
	&&&&\iff&& z&=2y-1&&&&&&&
\end{flalign*}
Notice that this equally means that $z=1+2\beps$ since
\begin{equation*}
	z=2y-1=2(1+\beps)-1=1+2\beps
\end{equation*}
\begin{lem}
Let $\mx$ be a schedule for $\inp$. There exists a $B$-restricted schedule $\mx'$ satisfying \makebox{$\costs(\mx')\le z\costs(\mx)$}.
\end{lem}
\begin{proof}
Let $\mx=(x_1,\dotsc,x_T)$ be a schedule for $\inp$. We need to construct a $B$-restricted schedule $\mx'=(x_1',\dotsc,x_T')$ such that \makebox{$\costs(\mx')\le z\costs(\mx)$}. By following the same reasoning as done for Lemma~\ref{lem:transform_schedule_approx_2}, we can again assume that $\mx$ is feasible and never shuts down all its servers. It then suffices to show that every period between two $y$-state changes of $\mx$ can be iteratively transformed to a $z$-approximative period in $\mx'$. 
...
We adapt the invariant used in Lemma~\ref{lem:transform_schedule_approx_2} in that we are going to ensure that $\mx'$ can potentially move to $\floor{u}$ at time $i$ in a $z$-approximative manner, i.e.\ we ensure that~$\mx'$ incurs at most $z$ times as much switching costs as $\mx$ if we set $x_i'\coloneqq \floor{u}$. Further, we are going to ensure that $x_t'\ge \floor{u}$ holds for any $i\le t\le j$ after every transformation step. This guarantees that $\mx'$ will be feasible, since $x_t\le \floor{u}$.

For the initial start-up process (i.e.\ $i=1$), we can simply set $x_1'\coloneqq \floor{u} \in B$. Since we know that $x_1'\le yx_1$, we can conclude that $\beta x_1'\le\beta yx_1\le\beta zx_1$; thus, the start-up switching costs of $\mx'$ are $y$-approximative and the invariant initially holds. The difference of switching costs $\beta(zx_1-x_1')=\beta(zx_1-\floor{u})\ge\beta(yx_1-\floor{u})$ can be used as a credit for our amortized analysis.

Now, let us have a closer look on the possible behaviors of $\mx$ between its $y$-state changes. Recall the four possible cases:
\begin{enumerate}[label=(\alph*)]
	\item $\mx$ enters $[l,u)$ from below and then descends to $[l',u')$.\label{itm:schedule_behavior_up_down_z}
	\item $\mx$ enters $[l,u)$ from above and then descends to $[l',u')$.\label{itm:schedule_behavior_down_down_z}
	\item $\mx$ enters $[l,u)$ from below and then ascends to $[l',u')$.\label{itm:schedule_behavior_up_up_z}
	\item $\mx$ enters $[l,u)$ from above and then ascends to $[l',u')$.\label{itm:schedule_behavior_down_up_z}
\end{enumerate}
To finish the proof, we need to show that any case can be transformed to a $z$-approximative period in $\mx'$ while ensuring that our invariant will hold at the beginning of the next period. Additionally, we have to verify that the credit of our amortized analysis stays non-negative. We will in particular show that our credit will be greater than or equal to $\beta(yx_{j+1}-\floor{u'})$ if $\mx$ exits $[l,u)$ by ascending to $[l',u')$ at time $j+1$. Since the periods are consecutive, this equivalently means that the credit will be greater than or equal to $\beta(yx_i-\floor{u})$ if $\mx$ enters $[l,u)$ at time~$i$ from below. Note that we already showed that this claim holds for our initial start-up process.

We start with cases~\ref{itm:schedule_behavior_up_down_z} and~\ref{itm:schedule_behavior_down_down_z}. Due to our invariant, we know that we can set $x_i'\coloneqq \floor{u}$ with $z$-approximative switching costs. Consequently, setting $x_i'\coloneqq x_{i+1}'\coloneqq\dotsb\coloneqq x_j'\coloneqq \floor{u}\in B$ gives us a strategy with $z$-approximative switching costs between $i$ and $j$. Further, since $x_t\le \floor{u}\le yx_t$ for any $i\le t\le j$, and $f$ is non-negative and monotonically increasing, the operating costs of $\mx'_{i,j}$ can be estimated by
\begin{align*}
	...
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}y\opcosts(\mx_{i,j})\le z\opcosts(\mx_{i,j})
\end{align*}
Thus, the operating costs of $\mx_{i,j}'$ are $z$-approximative. Further, since $\floor{u'}<\floor{u}$, we do not need to account for additional switching costs to satisfy our invariant at time $j+1$. Lastly, the credit of our amortized analysis stays untouched, i.e.\ it stays non-negative.
	
For case~\ref{itm:schedule_behavior_up_up_z}, we again know that setting \makebox{$x_i'\coloneqq x_{i+1}'\coloneqq\dotsb\coloneqq x_j'\coloneqq \floor{u}\in B$} gives us a strategy with $z$-approximative switching costs due to our invariant. Moreover, as in the previous case, the operating costs of $\mx_{i,j}'$ are $z$-approximative. To verify our invariant at time $j+1$, we use the fact that $\mx$ has to power on at least $x_{j+1}-x_i$ servers between~$i$ and $j+1$. Thus, we would have to show that $\beta(\floor{u'}-\floor{u})\le z\beta(x_{j+1}-x_i)$ holds. However, since we entered $[l,u)$ from below, we can additionally use our credit to compensate for our costs:
\begin{align*}
	z\beta(x_{j+1}-x_i)+\overbrace{\beta(yx_i-\floor{u})}^{\text{credit}}&\ge y\beta(x_{j+1}-x_i)+\beta(yx_i-\floor{u})\\
	&=\beta(yx_{j+1}-\floor{u})\ge\beta(\floor{u'}-\floor{u})
\end{align*}
...
Finally, we have to consider case~\ref{itm:schedule_behavior_down_up_z}, for which we again consider two different strategies, namely
\begin{flalign*}
...
We are now going to prove that either the cost of $\mx_{i,j}^{\floor{u}}$ or of $\mx_{i,j}^{\floor{\hat{u}}}$, including possible switching costs to satisfy our invariant at time $j+1$, must be $z$-approximative. 
First, we define $d\coloneqq\min\{x_t\mid i\le t\le j\}$ to be the smallest number of active servers used by $\mx_{i,j}$. We can use $d$ to decide which schedule we should use: If $\mx_{i,j}$ stays between $[o,u)$, we know that TODO by our conditions~\eqref{eq:conditions_z_o} and thus chose strategy $\mx_{i,j}^{\floor{\hat{u}}}$. If, on the other hand, $\mx_{i,j}$ crosses $o$, that is $x_t<o$ for some $i\le t\le j$, we choose strategy $\mx_{i,j}^{\floor{u}}$.

TODO bla blub Figure~\ref{fig:schedule_behavior_down_up_z}.
\begin{figure}[H]
\captionsetup[subfigure]{labelformat=empty}
\begin{subfigure}[b]{0.48\textwidth}
\includestandalone[width=\textwidth]{../figures/schedule_behavior_down_up_1_z}
\caption{Strategy $\mx_{i,j}^{\floor{u}}$ is chosen if $\mx_{i,j}$ crosses $o$ at least once.}
\end{subfigure}
\hfill
\begin{subfigure}[b]{0.48\textwidth}
\includestandalone[width=\textwidth]{../figures/schedule_behavior_down_up_2_z}
\caption{Strategy $\mx_{i,j}^{\floor{\hat{u}}}$ is chosen if $\mx_{i,j}$ stays between $[o,u)$.}
\end{subfigure}
\caption{The original schedule $\mx$ (case~\ref{itm:schedule_behavior_down_up_z}) comes from above, stays between $[l,u)$, and then rises to $[l',u')$. The approximative schedule is chosen in dependency of the interval used by $\mx_{i,j}$.}
\label{fig:schedule_behavior_down_up_z}
\end{figure}
Of course, we have to verify that both strategy give us a $z$-approximative period. First, if $\mx_{i,j}$ stays between $[o,u)$, we 
\begin{align*}
	z\beta(x_{j+1}-x_i)-\beta(\floor{u'}-\floor{\hat{u}})&\ge y\beta(x_{j+1}-x_i)-\beta(\floor{u'}-\floor{\hat{u}})\\
	&=\beta(yx_{j+1}-yx_i-\floor{u'}+\floor{\hat{u}})\ge\beta(yx_{j+1}-\floor{u'})
\end{align*}
oops\unsure{Fail: $yx_i>\floor{\hat{u}}$}
%where we assumed that $\hat{u}=2u$ holds for the last inequality; otherwise, we have $u'=\hat{u}=m$ and the inequality trivially holds. Further, if $\hat{u}=2u$, we can again use the difference of costs \makebox{$2\beta(x_{j+1}-u)-\beta(u'-\hat{u})=\beta(2x_{j+1}-u')$} as a credit for subsequent operations. If $\hat{u}=m$, no further credit is needed, since we already reached the limit of servers $m$.

TODO next case
\begin{align*}
	z\beta(x_{j+1}-x_i)-\beta(\floor{u'}-\floor{u})&\ge y\beta(x_{j+1}-x_i)-\beta(\floor{u'}-\floor{\hat{u}})\\
	&=\beta(yx_{j+1}-yx_i-\floor{u'}+\floor{\hat{u}})\ge\beta(yx_{j+1}-\floor{u'})
\end{align*}
...
\section{A $(1+\beps)$-Optimal Linear-Time Algorithm (Integer Failed)}
TODO text
...
For the initial start-up process (i.e.\ $i=1$), we can simply set $x_1'\coloneqq \floor{u} \in B$. Since we know that $x_1'\le yx_1$, we can conclude that $\beta x_1'\le\beta yx_1$; thus, the start-up switching costs of $\mx'$ are $y$-approximative and the invariant initially holds. The difference of switching costs $\beta(yx_1-x_1')=\beta(yx_1-\floor{u})$ can be used as a credit for our amortized analysis.
...
\section{A $(1+\beps)$-Optimal Linear-Time Algorithm (Non-Integer)}
TODO text
The proof of the next lemma will be very similar to that done in Lemma~\ref{lem:transform_schedule_approx_2}.
\begin{lem}
Let $\mx$ be a schedule for $\inp$. There exists a $B$-restricted schedule $\mx'$ satisfying \makebox{$\costs(\mx')\le y\costs(\mx)$}.
...
&&\text{and}&&u'\coloneqq\min\bigl\{yl',m\bigr\}&&&
\end{flalign*}
Note that $x_t\le u\le yx_t$ holds for any $i\le t\le j$.

We adapt the invariant used in Lemma~\ref{lem:transform_schedule_approx_2} in that we are going to ensure that $\mx'$ can potentially move to $u$ at time $i$ in a $y$-approximative manner, i.e.\ we ensure that~$\mx'$ incurs at most $y$ times as much switching costs as $\mx$ if we set $x_i'\coloneqq u$. Further, we are going to ensure that $x_t'\ge u$ holds for any $i\le t\le j$ after every transformation step. Since $x_t\le u$, this guarantees that $\mx'$ will be feasible.

For the initial start-up process (i.e.\ $i=1$), we can simply set $x_1'\coloneqq u \in B$. Since we know that $x_1'\le yx_1$, we can conclude that $\beta x_1'\le\beta yx_1$; thus, the start-up switching costs of $\mx'$ are $y$-approximative and the invariant initially holds. The difference of switching costs $\beta(yx_1-x_1')=\beta(yx_1-{u})$ can be used as a credit for our amortized analysis.
...
\begin{enumerate}[label=(\alph*)]
	...
	\item $\mx$ enters $[l,u)$ from above and then ascends to $[l',u')$.\label{itm:schedule_behavior_down_up_y} 
\end{enumerate}
To finish the proof, we need to show that any case can be transformed to a $y$-approximative period in $\mx'$ while ensuring that our invariant will hold at the beginning of the next period. Additionally, we have to verify that the credit of our amortized analysis stays non-negative. We will in particular show that our credit will be greater than or equal to $\beta(yx_{j+1}-{u'})$ if $\mx$ exits $[l,u)$ by ascending to $[l',u')$ at time $j+1$. Since the periods are consecutive, this equivalently means that the credit will be greater than or equal to $\beta(yx_i-{u})$ if $\mx$ enters $[l,u)$ at time~$i$ from below. We already showed that this claim holds for our initial start-up process.

We start with cases~\ref{itm:schedule_behavior_up_down_y} and~\ref{itm:schedule_behavior_down_down_y}. Due to our invariant, we know that we can set $x_i'\coloneqq {u}$ with $y$-approximative switching costs. Consequently, setting $x_i'\coloneqq x_{i+1}'\coloneqq\dotsb\coloneqq x_j'\coloneqq {u}\in B$ gives us a strategy with $y$-approximative switching costs between $i$ and $j$. Further, since $x_t\le {u}\le yx_t$ for any $i\le t\le j$, and $f$ is non-negative and monotonically increasing, the operating costs of $\mx'_{i,j}$ can be estimated by
\begin{align*}
	\opcosts(\mx_{i,j}')&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}\sum\limits_{t=i}^j\left({u} f\left(\frac{\lambda_t}{{u}}\right)\right)\le\sum\limits_{t=i}^j\left(yx_tf\left(\frac{\lambda_t}{{u}}\right)\right)\le y\sum\limits_{t=i}^j\left(x_tf\left(\frac{\lambda_t}{x_t}\right)\right)\\
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}y\opcosts(\mx_{i,j})
\end{align*}
Thus, the operating costs of $\mx_{i,j}'$ are $y$-approximative. Further, since ${u'}<{u}$, we do not need to account for additional switching costs to satisfy our invariant at time $j+1$. Lastly, the credit of our amortized analysis stays untouched, i.e.\ it stays non-negative.
	
For case~\ref{itm:schedule_behavior_up_up_y}, we again know that setting \makebox{$x_i'\coloneqq x_{i+1}'\coloneqq\dotsb\coloneqq x_j'\coloneqq {u}\in B$} gives us a strategy with $y$-approximative switching costs due to our invariant. Moreover, as in the previous case, the operating costs of $\mx_{i,j}'$ are $y$-approximative. To verify our invariant at time $j+1$, we use the fact that $\mx$ has to power on at least $x_{j+1}-x_i$ servers between~$i$ and $j+1$. Thus, we would have to show that $\beta({u'}-{u})\le y\beta(x_{j+1}-x_i)$ holds. However, since we entered $[l,u)$ from below, we can additionally use our credit to compensate for our costs:
\begin{align*}
	y\beta(x_{j+1}-x_i)+\overbrace{\beta(yx_i-{u})}^{\text{credit}}=\beta(yx_{j+1}-{u})\ge\beta({u'}-{u})
\end{align*}
where the last inequality follows from ${u'}\le yx_{j+1}$. Thus, our invariant at time $j+1$ is satisfied. The difference $\beta(yx_{j+1}-{u})-\beta({u'}-{u})=\beta(yx_{j+1}-{u'})$ can further be used as our new credit. 
...
\begin{flalign*}
	...
	&&\text{and}\quad\mx_{i,j}^{{\hat{u}}}&\coloneqq\bigl(x_i^{{\hat{u}}},\dotsc,x_j^{{\hat{u}}}\bigr),&&\text{where}&x_t^{{\hat{u}}}\coloneqq {\hat{u}},\,\text{for } i\le t\le j&&&&
\end{flalign*}
with $\hat{u}\coloneqq\min\{yu,m\}$. Due to our invariant and the fact that $\mx$ descends at time $i$, we know that $x_{i-1}'\ge{\hat{u}}\ge{u}$, and thus both strategies do no incur switching costs up to time~$j$.
We are now going to prove that either the cost of $\mx_{i,j}^{{u}}$ or of $\mx_{i,j}^{{\hat{u}}}$, including possible switching costs to satisfy our invariant at time $j+1$, must be $y$-approximative. 
First, we define $d\coloneqq\min\{x_t\mid i\le t\le j\}$ to be the smallest number of active servers used by $\mx_{i,j}$. We can use $d$ to give a lower bound for the switching costs of $\mx$ between $i$ and~$j+1$:
\begin{equation*}
	\sum\limits_{t=i+1}^{j+1}\swcosts(x_{t-1},x_t)\ge\beta(x_{j+1}-d)
\end{equation*}
Then, we again split $\mx_{i,j}$ and its associated switching costs at the final switching step into two parts, i.e.\ we split $\beta(x_{j+1}-d)$ into $\beta(x_{j+1}-u)$ and $\beta(u-d)$.

It now suffices to show that either $\mx_{i,j}^u$ or $\mx_{i,j}^{\hat{u}}$ can process the loads $\lambda_i,\dotsc,\lambda_j$ and eventually switch to ${\hat{u}}$ with $y$-approximative costs under the assumption that $\mx_{i,j}$ only switches to~$u$ as a first step. This is due to the fact that the remaining switching costs to ascend from ${\hat{u}}$ to ${u'}$ are then $y$-approximative anyway:
\begin{equation*}
	\beta({u'}-{\hat{u}})\le\beta(yx_{j+1}-{\hat{u}})=y\beta(x_{j+1}-u)
\end{equation*}
where we assumed that $\hat{u}=yu$ holds; otherwise, we have $u'=\hat{u}=m$ and the inequality trivially holds. 
Further, we can then again use the difference of costs for our credit:
\begin{align*}
	y\beta(x_{j+1}-u)-\beta(u'-\hat{u})&=\beta(yx_{j+1}-u-u'+\hat{u})\\
	&=\beta(yx_{j+1}-yu-u'+yu)=\beta(yx_{j+1}-u')
\end{align*}
If $\hat{u}=m$, no further credit is needed, since we already reached the limit of servers $m$.

To proceed, we notice that if $\hat{u}-u\le y(u-d)$ holds, strategy $\mx_{i,j}^u$ has 2-approximative switching costs:
\begin{equation*}
	\beta(\hat{u}-u)\le y\beta(u-d)
\end{equation*}
Further, as in the previous cases, the operating costs of $\mx_{i,j}^u$ are $y$-approximative, and thus~$\mx_{i,j}^u$ is $2$-approximative if $\hat{u}-u\le2(u-d)$ holds. Hence, we subsequently assume that 
\begin{align}\label{eq:proof_y_approximative_asm_dist_pos}
	\hat{u}-u>y(u-d)&&\text{or equivalently}&&\hat{u}-(y+1)u+yd>0
\end{align}
Next, we can assume that $\mx_{i,j}^{{u}}$ is not $y$-approximative; otherwise, we are already done. Hence, assume that 
\begin{equation*}
	\opcosts(\mx_{i,j}^u)+\beta(\hat{u}-u)>y\bigl(\opcosts(\mx_{i,j})+\beta(u-d)\bigr)
\end{equation*}
We then have to show that $\mx_{i,j}^{{\hat{u}}}$ is $y$-approximative.
Rearranging the previous inequality gives us an estimation for $\beta$:
\begin{align*}
	&&\opcosts(\mx_{i,j}^u)+\beta(\hat{u}-u)&>y\bigl(\opcosts(\mx_{i,j})+\beta(u-d)\bigr)&\\
	&\stackrel{\phantom{\hat{u}-(y+1)u+yd)>0}}{\iff}&\beta(\hat{u}-(y+1)u+yd)&>y\opcosts(\mx_{i,j})-\opcosts(\mx_{i,j}^u)&\\
	&\stackrel{\hat{u}-(y+1)u+yd>0}{\iff}&\beta&>\frac{y\opcosts(\mx_{i,j})-\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}
\end{align*}
where the last step is justified by Assumption~\eqref{eq:proof_y_approximative_asm_dist_pos}. This estimation allows us to find a lower bound for the cost of $\mx_{i,j}$:
\begin{align*}
	\opcosts(\mx_{i,j})+\beta(u-d)&>\opcosts(\mx_{i,j})+\frac{y\opcosts(\mx_{i,j})-\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}(u-d)\\
	&=\frac{\bigl(\hat{u}-(y+1)u+yd\bigr)\opcosts(\mx_{i,j})+y(u-d)\opcosts(\mx_{i,j})-(u-d)\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}\\
	&=\frac{(\hat{u}-u)\opcosts(\mx_{i,j})-(u-d)\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}
\end{align*}
Next, since we want to prove that $\opcosts(\mx_{i,j}^{{\hat{u}}})$ is $y$-approximative, we have to show that the following cost difference is non-negative:
\begin{align*}
	y\bigl(\opcosts(\mx_{i,j})+\beta(u-d)\bigr)-\opcosts(\mx_{i,j}^{\hat{u}})
\end{align*}
Recall that $\mx_{i,j}^{{\hat{u}}}$ does not incur switching costs to move to ${\hat{u}}$. We can now use our just derived lower bound for $\opcosts(\mx_{i,j})+\beta(u-d)$ to estimate the difference:
\begin{align*}
	&y\bigl(\opcosts(\mx_{i,j})+\beta(u-d)\bigr)-\opcosts(\mx_{i,j}^{\hat{u}})
-\opcosts(\mx_{i,j}^{{\hat{u}}})\\
	>\quad&y\, \frac{(\hat{u}-u)\opcosts(\mx_{i,j})-(u-d)\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}-\opcosts(\mx_{i,j}^{{\hat{u}}})
\end{align*}
Thus, it suffices to show that
\begin{align*}
	y\, \frac{(\hat{u}-u)\opcosts(\mx_{i,j})-(u-d)\opcosts(\mx_{i,j}^u)}{\hat{u}-(y+1)u+yd}-\opcosts(\mx_{i,j}^{{\hat{u}}})\ge 0
\end{align*}
which can be simplified to
\begin{equation*}
	y(\hat{u}-u)\opcosts(\mx_{i,j})-y(u-d)\opcosts(\mx_{i,j}^u)-\bigl(\hat{u}-(y+1)u+yd\bigr)\opcosts(\mx_{i,j}^{{\hat{u}}})\ge 0
\end{equation*}
To show this inequality, we have to take a closer look on the the schedules' operating costs:
\begin{align*}
	&&&y({\hat{u}}-{u})\opcosts(\mx_{i,j})-y(u-d)\opcosts(\mx_{i,j}^{{u}})-\bigl(\hat{u}-(y+1)u+yd\bigr)\opcosts(\mx_{i,j}^{{\hat{u}}})\\
	&\stackrel{\eqref{eq:mx_schedule_op_costs_complete}}{=}&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})x_tf\left(\frac{\lambda_t}{x_t}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{\hat{u}}}\right)\right)
\end{align*}
...
\begin{align*}
	&&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})x_tf\left(\frac{\lambda_t}{x_t}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{\hat{u}}}\right)\right)\\
	&\ge&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})x_tf\left(\frac{\lambda_t}{u}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{u}}\right)\right)
\end{align*}
Since $d$ is the smallest number of active servers scheduled by $\mx$, and $f$ is non-negative, we can conclude that
\begin{align*}
	&&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})x_tf\left(\frac{\lambda_t}{u}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{u}}\right)\right)\\
	&\ge&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})df\left(\frac{\lambda_t}{u}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{u}}\right)\right)
\end{align*}
Hence, to finish the case, it suffices to show that
\begin{align*}
	\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})df\left(\frac{\lambda_t}{u}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{u}}\right)\right)\ge 0
\end{align*}
Further rearranging the left hand side gives us
\begin{align*}
	&&&\sum\limits_{t=i}^j\left(y({\hat{u}}-{u})df\left(\frac{\lambda_t}{u}\right)-y(u-d){u}f\left(\frac{\lambda_t}{{u}}\right)-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}f\left(\frac{\lambda_t}{{u}}\right)\right)\\
	&=&&\sum\limits_{t=i}^j\left(\Bigl(y({\hat{u}}-{u})d-y(u-d)u-\bigl(\hat{u}-(y+1)u+yd\bigr){\hat{u}}\Bigr)f\left(\frac{\lambda_t}{{u}}\right)\right)\\
	&=&&\sum\limits_{t=i}^j\left(\Bigl(yd{\hat{u}}-yd{u}-yu^2+yd{u}-\hat{u}^2+(y+1)u\hat{u}-yd\hat{u}\Bigr)f\left(\frac{\lambda_t}{{u}}\right)\right)\\
	&=&&\sum\limits_{t=i}^j\left(\Bigl(-\hat{u}^2+(y+1)u\hat{u}-yu^2\Bigr)f\left(\frac{\lambda_t}{{u}}\right)\right)=\sum\limits_{t=i}^j\left(\Bigl(-(u-\hat{u})(yu-\hat{u})\Bigr)f\left(\frac{\lambda_t}{{u}}\right)\right)
\end{align*}
Now let $g(\hat{u})\coloneqq-(u-\hat{u})(yu-\hat{u})$. Since $f$ is non-negative, it suffices to show that $g(\hat{u})\ge0$ for all possible values of $\hat{u}$, namely $\hat{u}\in[u,yu]$. We notice that $g(\cdot)$ is an inverted parabola with roots $u$ and $yu$. Thus, we know that $g(u)=g(yu)=0$ and $g(\hat{u})>0$ for $u<\hat{u}<yu$, which finishes the case.
