...
\end{align*}
Thus, the operating costs of $\mx_{i,j}'$ are $y$-approximative. Further, since $\floor{u'}<\floor{u}$, we do not need to account for additional switching costs to satisfy our invariant at time~$j+1$. Lastly, the credit of our amortized analysis stays untouched, i.e.\ it stays non-negative. In particular, the credit stays at $\beta(yx_i-\floor{u})$ for case~\ref{itm:schedule_behavior_up_down_y} -- which we will have to use for case~\ref{itm:schedule_behavior_down_up_y}.
...
\end{align*}
which finishes the case with assumption $yd\le\floor{\hat{u}}$. Now assume that $yd>\floor{\hat{u}}$.
TODO
