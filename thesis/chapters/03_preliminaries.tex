...
\end{itemize}
\end{proof}
Next, we consider the sequence of the number of active servers $\mx$ defined as
\begin{flalign*}
...
Given a problem instance $\inp$, there exists an optimal schedule $\Sigma^*$ that evenly distributes its arrival rates to its active servers in each time slot and further satisfies~\eqref{eq:reasonable_switching} for all $t\in[T]$.
\end{thm}
\begin{proof}
By Corollary~\ref{cor:even_load_distribution}, we obtain an optimal schedule $\Sigma$ that evenly distributes its arrival rates to its active servers. Applying the procedure given in Proposition~\ref{prop:reasonable_switching} to $\Sigma$ yields $\Sigma^*$ that further satisfies~\eqref{eq:reasonable_switching} for all $t\in[T]$.
\end{proof}

Theorem~\ref{thm:even_load_distribution_reasonable_switching} allows us to identify an optimal schedule by its sequence of active servers $\mx$ and thereby to simplify our optimization conditions~\eqref{eq:schedule_costs} and \eqref{eq:feasible_constraint}. 
...
\begin{equation}\label{eq:mx_schedule_op_costs}
	\opcosts:[m]_0\times[0,m]\rightarrow\mathbb{R}\cup\{\infty\},\quad \opcosts(x,\lambda)=\begin{cases}
