...
\begin{algorithm}[H]
  ...
  \Function{2\_optimal\_offline\_scheduling}{$m,T,\Lambda,\beta,f$}
	  \Let{$b$}{$\log_2(\lceil m\rceil)$}
	  \Let{$(C,P)$}{\Call{shortest\_paths}{$m,T,\Lambda,\beta,f,b$}}
	  \Let{$\mx$}{\Call{extract\_schedule}{$P,T,m,b$}}
	  \State \Return{$\mx$}
  \EndFunction
  %\Statex
  %\Function{node}{$i,m,b$}
	  %\If{$i=b+1$}
		%\Return{$m$}
	  %\EndIf
	  %\State \Return{$\lfloor2^{i-1}\rfloor$}
  %\EndFunction
  \algstore{lin2opt}
  \end{algorithmic}
\end{algorithm}
\begin{algorithm}[H]
  \begin{algorithmic}[1]
  \algrestore{lin2opt}
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f,b$}
	\LineCommentFunc{Allocate nodes' costs and predecessor tables}
	...
	\For{$i \gets b \textrm{ to } 0$}\Comment{Initialize first layer (downward minimization step)}
		\If {$C[i+1,1]<\costs\bigl(0,\Call{node}{i,m,b},\lambda_1\bigr)$}
			\Let{$P[i,1]$}{$P[i+1,1]$ and $C[i,1]\gets C[i+1,1]$}
		\Else
			\Let{$P[i,1]$}{$i$ and $C[i,1]\gets\costs\bigl(0,\Call{node}{i,m,b},\lambda_1\bigr)$}
		\EndIf
	\EndFor
	\For{$t \gets 1 \textrm{ to } T-1$}\Comment{Iterative calculate costs and predecessors}
		\For{$i \gets 1 \textrm{ to } b+1$}\Comment{Upward minimization step}
			\If {$C[i-1,t]+\beta\bigl(\Call{node}{i,m,b}-\Call{node}{i-1,m,b}\bigr)<C[i,t]$}
			    \Let{$P[i,t]$}{$P[i-1,t]$}
			    \Let{$C[i,t]$}{$C[i-1,t]+\beta\bigl(\Call{node}{i,m,b}-\Call{node}{i-1,m,b}\bigr)$}
			\EndIf
		\EndFor
		...
		\For{$i \gets b \textrm{ to } 0$}\Comment{Downward minimization step}
			\If {$C[i+1,t+1]<C[i,t]+\opcosts\bigl(\Call{node}{i,m,b},\lambda_{t+1}\bigr)$}
				\Let{$P[i,t+1]$}{$P[i+1,t+1]$ and $C[i,t+1]\gets C[i+1,t+1]$}
			\Else
				\Let{$P[i,t+1]$}{$i$ and $C[i,t+1]\gets C[i,t]+\opcosts\bigl(\Call{node}{i,m,b},\lambda_{t+1}\bigr)$}
			\EndIf
		\EndFor
	\EndFor
	\State \Return{$(C,P)$}
