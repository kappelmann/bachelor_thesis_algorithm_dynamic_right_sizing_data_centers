...
\begin{algorithm}[H]
  ...
  \Function{2\_optimal\_offline\_scheduling}{$m,T,\Lambda,\beta,f$}
	  ...
	  \Let{$\mx$}{\Call{extract\_schedule}{$P,T,b$}}
	  \State \Return{$\mx$}
  \EndFunction
  \Statex
  \Function{node}{$i$}
...
\begin{algorithm}[H]
  ...
  \Function{shortest\_paths}{$m,T,\Lambda,\beta,f,b$}
	\LineComment{Allocate nodes' costs and predecessors tables}
	...
	\For{$i \gets b \textrm{ to } 0$}\Comment{Initialize first layer (downward minimization step)}
		\If {$C[i+1,1]<\costs\bigl(0,\Call{node}{i},\lambda_1\bigr)$}
			\Let{$P[i,1]$}{$P[i+1,1]$ and $C[i,1]\gets C[i+1,1]$}
		\Else
			\Let{$P[i,1]$}{$i$ and $C[i,1]\gets\costs\bigl(0,\Call{node}{i},\lambda_1\bigr)$}
		\EndIf
	\EndFor
	\For{$t \gets 1 \textrm{ to } T-1$}\Comment{Iterative calculate costs and predecessors}
		\For{$i \gets 1 \textrm{ to } b+1$}\Comment{Upward minimization step}
			\If {$C[i-1,t]+\beta\cdot\Call{node}{i}<C[i,t]$}
  ...
  \EndFunction
  \Statex
  \Function{extract\_schedule}{$P,T,b$}
