...
\begin{flalign*}
	&&l'\coloneqq 2^{\lfloor\log_2(x_{j+1})\rfloor}&&\text{and}&&u'\coloneqq\min\bigl\{2^{\lceil\log_2(x_{j+1}+1)\rceil},m\bigr\}&&&
\end{flalign*}
...
\end{figure}
We are now going to prove that either $\mx_{i,j}^u$ or $\mx_{i,j}^{\hat{u}}$, including possible switching costs to satisfy our invariant for $j+1$, must be 2-competitive. If $\mx_{i,j}^u$ is 2-competitive, we are done; hence, assume that this is not the case. We then need to show that $\mx_{i,j}^{\hat{u}}$ is 2-competitive. First, we examine the switching costs of $\mx$ from $i$ up to $j+1$. Since $\mx$ has to power on at least $x_{j+1}-\min\{x_t\mid i\le t\le j\}$ servers between $i$ and $j+1$, we can estimate its switching costs by
\begin{align*}
	\sum\limits_{t=i+1}^{j+1}\swcosts(x_{t-1},x_t)\ge\beta\bigl(x_{j+1}-\min\{x_t\mid i\le t\le j\}\bigr)
\end{align*}
By assumption, we know that $\mx_{i,j}^u$ is not 2-competitive, that is we have
\begin{equation*}
	\opcosts(\mx_{i,j}^l)+\beta(u-l)>2\bigl(\opcosts(\mx_{i,j})+\beta d\bigr)
\end{equation*}
