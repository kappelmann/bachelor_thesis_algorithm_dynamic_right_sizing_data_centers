...
For any possible amount of active servers $i$ and any time slot $t$ we add a node $v_{i,t}$. Moreover, we add a start node $v_{0,0}$ as well as an end node $v_{0,T+1}$. Next, we connect all nodes to their successors with respect to time. Semantically, $v_{i,t}$ denotes the state of scheduling the arrival rate $\lambda_{t}$ equally to $i$ servers. For any edge connecting $v_{i,t}$ with $v_{j,t+1}$ we assign costs $\costs(i,j,\lambda_{t+1})$, i.e.\ the costs of taking the edge correspond to switching from $i$ to $j$ machines and processing the load $\lambda_{t+1}$ with $j$ machines.

The costs of a path $P=(v_{i_0,t},v_{i_1,t+1},\ldots,v_{i_n,t+n})$ (where $i_k\in\fromto{0}{m}$) of length $n$ in our graph are thus given by\unsure{Is this well-written?}
\begin{equation*}
	\costs(P)\coloneqq\sum\limits_{k=1}^{n}\costs(i_{k-1},i_k,\lambda_{t+k})
\end{equation*}
In particular, for a path $P=(v_{0,0},v_{i_1,1},\ldots,v_{i_T,T},v_{0,T+1})$ from $v_{0,0}$ to $v_{0,T+1}$ we have
\begin{equation*}
	\costs(P)=c(0,i_1,\lambda_1)+\sum\limits_{k=2}^{T}\costs(i_{k-1},i_{k},\lambda_{k})+\underbrace{c(i_T,0,0)}_{=0}=c(0,i_1,\lambda_1)+\sum\limits_{k=2}^{T}\costs(i_{k-1},i_{k},\lambda_{k})
\end{equation*}
Naturally, any shortest path from $v_{0,0}$ to $v_{0,T+1}$ is forced to minimise the costs of the corresponding schedule due to our construction. Needless to say, this demands for a proof of correctness.
\begin{figure}[H]
\centering
\scalebox{0.85}{
...
\caption{Level structured graph for an optimal offline algorithm. Remark that $\costs(i,0,\lambda_t)=0$ for all $i\in\fromto{0}{m}$ and $t\in[T+1]$ and that $\lambda_{T+1}=0$ by our conventions.}
\end{figure}
\begin{prop}
Any given optimal schedule $\mx$ can be transformed to a shortest path $P$ from $v_{0,0}$ to $v_{0,T+1}$ with $\costs(\mx)=\costs(P)$ and vice versa.
\end{prop} 
\begin{proof}
$ $
\begin{itemize}
