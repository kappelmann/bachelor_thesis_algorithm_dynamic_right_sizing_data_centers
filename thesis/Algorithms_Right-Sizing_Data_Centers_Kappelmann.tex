...
The incurring energy costs of a single machine can be described by the sum of the machine's \textit{(power state) switching costs}, specified by $\beta\in\mathbb{R}_{\ge 0}$, as well as its \textit{operating costs}, specified by \makebox{$f:[0,1]\rightarrow \mathbb{R}$}. We assume that a sleeping server does not generate any energy costs. Note that $f(0)$ describes the costs generated by an idle server, not a sleeping one; in particular, $f(0)$ may be non-zero. Further, we assume convexity for $f$. This may seem like a notable restriction at first, but it indeed captures the behaviour of most modern server models. Since we are dealing with homogeneous servers, $\beta$ and $f$ are the same for all machines.
...
Given an input $\inp$, our goal is to find a schedule $\Sigma$ that satisfies the following optimisation:\unsure{Definition of $c(\Sigma)$ too hidden?}
...
For this, we first define the operating costs function $\opcosts(x,\lambda)$. Given a problem instance $\inp$, $\opcosts(x,\lambda)$ describes the operating costs incurred by equally distributing $\lambda$ on $x$ active servers using $f$:
\begin{equation*}
...
\begin{align*}
	\text{minimise}\quad\underbrace{\sum\limits_{t=1}^{T}\opcosts(x_t,\lambda_t)}_{\text{operating costs}}+\underbrace{\beta*\sum\limits_{t=1}^{T}\min\{0,x_t-x_{t-1}\}}_{\text{switching costs}}
\end{align*}
...
In this section, we derive an optimal offline algorithm based on our preliminary work. We reduce our scheduling problem instance $\inp$ to a shortest path problem of a level structured graph $G$. We can then use a dynamic programming approach to find a shortest path of $G$ and subsequently an optimal schedule for $\inp$ in pseudo-polynomial time.
