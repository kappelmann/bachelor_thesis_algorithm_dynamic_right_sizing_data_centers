...
\begin{algorithm}[H]
	...
	\For{$t \gets 2 \textrm{ to } T$}\Comment{Iterative calculation of costs and parents}
		\For{$x' \gets 0 \textrm{ to } m$}
			\Let{$C[t,x']$}{$\min\bigl\{C[t-1,x]+c(x,x',\lambda_t)\mid x\in\fromto{0}{m}\bigr\}$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
   \Function{Extract\_optimal\_offline}{$m,M,p,T$}
	\Blet{$\mx[T]$}{a new array}
	    \Let{$x[T]$}{$\argmin\limits_{x\in\fromto{0}{m}}\bigl\{M[T,x]\bigr\}$}\Comment{Initialisation; find minimum at last time slot}
	    \For{$t \gets T-1 \textrm{ to } 1$}\Comment{Iteratively obtain schedule from parents array}
		\Let{$x[t]$}{$p\bigl[t+1,x[t+1]\bigr]$}
	\EndFor
	\State \Return{$x$}
  \EndFunction
  \end{algorithmic}
...
Algorithm~\ref{alg_opt_offline} needs $\mathcal{O}(m)$ iterations for its initialisation and $\mathcal{O}(Tm^2)$ steps for the iterative calculation. Algorithm~\ref{alg_extr_opt_offline} needs $\mathcal{O}(T)$ steps for its initialisation and for its iterative calculation, respectively. Thus, we receive a run time of 
\begin{equation*}
	\mathcal{O}(Tm+Tm^2+2T)=\mathcal{O}(Tm^2)
\end{equation*}
