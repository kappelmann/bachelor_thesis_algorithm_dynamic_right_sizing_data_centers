...
For any possible amount of active servers $x$ and any time slot $t$ we add a node $v_{x,t}$. Moreover, we add a start node $v_{0,0}$ as well as an end node $v_{0,T+1}$. Next, we connect all nodes to their successors with respect to time. Semantically, $v_{x,t}$ denotes the state of scheduling the arrival rate $\lambda_{t}$ equally to $x$ servers at time slot $t$. For any edge connecting $v_{x,t}$ with $v_{x',t+1}$ we assign costs $\costs(x,x',\lambda_{t+1})$, i.e.\ the costs of taking the edge correspond to switching from $x$ to $x'$ machines and processing the load $\lambda_{t+1}$ with $x'$ machines.
...
\textproc{shortest\_paths} uses a dynamic programming approach similar to the well-known Bellman-Ford algorithm. It returns the minimum costs to all vertices as well as the predecessor of any vertex in respect to its shortest path.
...
\textproc{shortest\_paths} needs $\Theta(m)$ iterations for its initialisation, $\Theta(Tm^2)$ steps for the iterative calculation process, and $\Theta(m)$ steps for its final minimisation search. In addition, \textproc{extract\_schedule} needs $\Theta(T)$ iterations for its schedule retrieval. Thus, we receive a runtime of 
\begin{equation*}
...
\unsure{Is this well written?}Let us revise our initial algorithm. For any vertex $v_{i,t}$ of $G$, the algorithm calculates the minimum cost to $v_{i,t}$. This cost orresponds to the lowest achievable cost up to time slot $t$ of all schedules $\mx$ that assign $i$ servers at time $t$ to process the arrival rate $\lambda_t$. In particular, the cost of the end node $v_{0,T+1}$ tell us the minimum cost of all schedules. This approach, however, does not consider the possibility to schedule $j\neq i$ servers at time slot $t$ and to switch to $i$ servers just at the very last moment of $t$ when calculating the cost of $v_{i,t}$. Consider the following two examples:
\begin{figure}[H]
...
\end{tikzpicture} }}
\caption{Two examples depicting the shortcoming of our initial approach. The calculated vertices' costs are highlighted in red. Dashed edges are not part of the initial algorithm.}
\end{figure}
Although our algorithm delivers the correct end results, its immediate steps are somewhat unsatisfying. In practice, we can reach a state $v_{i,t}$ by moving down from a state $v_{j,t}$ where $j>i$ with cost $0$ or by moving up from a state $v_{j,t}$ where $j<i$ with costs $\beta*(i-j)$.

For this, we add local minimisation steps after any time slot. 
These minimisation steps will allow us to eliminiate the need of considering any possible scheduling choice at any time slot. Rather, we just need to 
\begin{align*}
	...
	&E_{\downarrow\uparrow}\coloneqq\bigl\{(v_{x,t\downarrow},v_{x,t+1\uparrow})\mid x\in\fromto{0}{m},t\in[T]\bigr\}\\
...
We modify our graph from section~\ref{sec_opt_offline_pseudo_poly} to the reduce the number of vertices. For this, we stop adding m vertices for each timestep, but use vertices that approximate the number of active servers instead. First, let $b\coloneqq\lceil\log_2(m)\rceil$. We add vertices $(t,0)$ and $(t,2^i),\forall t\in[T-1],0\le i\le b$. All edges and weights are added analogous to chapter~\ref{sec_opt_offline_pseudo_poly}.
\begin{figure}[H]
\begin{tikzpicture}[->,>=stealth',auto,node distance=3cm,thick,node/.style={minimum size=1.2cm,circle,draw}]
