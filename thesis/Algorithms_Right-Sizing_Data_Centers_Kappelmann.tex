...
Given a schedule $\Sigma$ with $x_t\in\mathbb{N}$ active servers at time slot $t$, an arrival rate $\lambda_t\in[0,x_t]$, and a convex cost function $f$, a most cost-efficient and feasible scheduling strategy is to assign each active server a load of $\lambda_t/x_t$.\unsure{This is sounds awkwardly formulated, doesn't it?}
\end{prop}
...
\begin{equation*}
	c_{op}:\fromto{0}{m}\times[0,m]\rightarrow\mathbb{R},\quad(x,\lambda)\mapsto\begin{cases}
          0, & \text{if $x=0$}\\
...
\begin{align*}
	\text{minimise}\quad\underbrace{\sum\limits_{t=1}^{T}c_{op}(x_t,\lambda_t)}_{\text{operating costs}}+\underbrace{\beta*\sum\limits_{t=1}^{T}\min\{0,x_t-x_{t-1}\}}_{\text{switching costs}}
\end{align*}
