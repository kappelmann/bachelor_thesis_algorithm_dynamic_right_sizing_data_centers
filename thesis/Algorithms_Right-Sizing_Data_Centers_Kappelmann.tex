...
\caption{Level structured graph for an optimal offline algorithm. Note that $\costs(i,0,\lambda_{T+1})=0$ for all $i\in\fromto{0}{m}$ because $\lambda_{T+1}=0$ by our conventions.}
\end{figure}
...
\begin{algorithm}[H]
	...
	\For{$t \gets 2 \textrm{ to } T$}\Comment{Iterative calculation of costs and parents}
		\For{$x' \gets 0 \textrm{ to } m$}
			\Let{$p[t,x']$}{$\argmin\limits_{x\in\fromto{0}{m}}\bigl\{C[t-1,x]+c(x,x',\lambda_t)\bigr\}$}\Comment{Find best preceding choice}
			\Let{$C[t,x']$}{$C\bigl[t-1,p[t,x']\bigr]+c\bigl(p[t,x'],x',\lambda_t\bigr)$}
		\EndFor
	\EndFor
	\Let{$p[T+1,0]$}{$\argmin\limits_{x\in\fromto{0}{m}}\bigl\{C[T,x]\bigr\}$}\Comment{Find best choice at last time slot}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
   \Function{Extract\_optimal\_offline}{$C,p,m,T$}
	\Blet{$\mx[T]$}{a new array}
    	\Let{$\mx[T]$}{$p[T+1,0]$}\Comment{Get minimum at last time slot}
