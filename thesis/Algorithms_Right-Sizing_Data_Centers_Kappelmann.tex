...
\end{proof}
As a result of Proposition~\ref{prop_path_to_reasn_path}, we shall subsequently focus our attention on reasonable paths. Intuitively, one can see that such a path $P$ can be uniquely identified by the ``maximum'' nodes $v_{x,t\downarrow}$ taken by $P$ at any time slot $t$. The next proposition formalises this intuition.
\begin{prop}
...
Next, let $(x_1,\ldots,x_T)\in\bm{\mx}$ be arbitrary. For any $t\in[T]$, we set
\begin{equation*}
	S'\coloneqq
	\begin{cases}
		(v_{x_s,t\downarrow},v_{x_s,t\uparrow},v_{x_s+1,t\uparrow},\ldots,v_{x_e,t\uparrow}), & \text{if $x_s< x_e$}\\
...
\end{lem}
\begin{proof}
By Proposition~\ref{prop_reasn_paths_identify}. Let $P$ be a reasonable path.
