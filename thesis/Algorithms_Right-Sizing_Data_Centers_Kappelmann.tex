...
\begin{itemize}
\item\underline{Basis:} For $n=1$ we have $x_1=1$ and thus
\begin{equation*}
...
\begin{itemize}
	...
	\item\underline{Proof:} Since $\sum\limits_{i=1}^{n+1}x_i=1$, at least one $x_i$ must be positive. Without loss of generality, we may assume $x_1>0$.
	\begin{equation*}
		f\Bigl(\sum\limits_{i=1}^{n+1}x_i\lambda_i\Bigr)=f\Bigl(x_1\lambda_1+\sum\limits_{i=2}^{n+1}x_i\lambda_i\Bigr)\stackrel{x_1\neq 0}{=}f\Bigl(x_1\lambda_1+(1-x_1)\sum\limits_{i=2}^{n+1}\frac{x_i\lambda_i}{1-x_1}\Bigr)
	\end{equation*}
	As $f$ is convex, we have
	\begin{equation*}
		f\Bigl(x_1\lambda_1+(1-x_1)\sum\limits_{i=2}^{n+1}\frac{x_i\lambda_i}{1-x_1}\Bigr)\stackrel{\text{f convex}}{\le} x_1f(\lambda_1)+(1-x_1)f\Bigl(\sum\limits_{i=2}^{n+1}\frac{x_i\lambda_i}{1-x_1}\Bigr)
	\end{equation*}
	Since $\sum\limits_{i=2}^{n+1}\frac{x_i}{1-x_1}=1$, we can apply our induction hypothesis.
	\begin{equation*}
		x_1f(\lambda_1)+(1-x_1)f\Bigl(\sum\limits_{i=2}^{n+1}\frac{x_i\lambda_i}{1-x_1}\Bigr)\stackrel{\text{I.H.}}{\le} x_1f(\lambda_1)+(1-x_1)\sum\limits_{i=2}^{n+1}\frac{x_i}{1-x_1}f(\lambda_i)
	\end{equation*}
	We combine our steps and obtain
	\begin{align*}
		f\Bigl(\sum\limits_{i=1}^{n+1}x_i\lambda_i\Bigr)&\le x_1f(\lambda_1)+(1-x_1)\sum\limits_{i=2}^{n+1}\frac{x_i}{1-x_1}f(\lambda_i)\\
		&=x_1f(\lambda_1)+\sum\limits_{i=2}^{n+1}x_if(\lambda_i)=\sum_{i=1}^{n+1} x_i f(\lambda_i)\qedhere
	\end{align*}
\end{itemize}
\end{itemize}
\end{proof}
