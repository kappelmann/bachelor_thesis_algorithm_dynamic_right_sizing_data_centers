...
Using these sequences $S_i$ and $L_i$, we can now define the sequence of all state changes and the sequence of all assigned loads:\unsure{good sentence?\\$\in(\{0,1\}^m)^T$} 
...
We will subsequently call a pair $\Sigma\coloneqq(\mathcal{S},\mathcal{L})$ a \textit{schedule}. Given an input $\inp$, our goal is to find a schedule $\Sigma$ that satisfies the following optimisation:\unsure{Definition of $c(\Sigma)$ too hidden?}
