...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.2cm,thick,node/.style={minimum size=1.5cm,circle,draw}]
  ...
  \node[node] (2) [above right=4cm of 1] {3,1$\downarrow$};
  ...
  \node[node] (19) [right =of 15] {1,T$\downarrow$};
  \node[node] (18) [right =of 14] {3,T$\downarrow$};
  \node[node] (20) [right =of 16] {0,T$\downarrow$};
  \node[node] (21) [right =of 17] {2,T$\downarrow$};

  \path[every node/.style={font=\sffamily\small}]
    (1) edge[red] node[black, above left] {$\costs(0,3,3)$} (2)
	edge node[label={[xshift=0.78cm, yshift=-0.8cm]$\costs(0,2,3)$}] {} (5)
	edge node[above right=-0.08cm] {$\costs(0,1,3)$} (3)
...
The next lemma formalises these ideas.
\begin{prop}
Let $P$ be a reasonable path. For any $t\in[T]$, let $V_\downarrow^t$ be the set of nodes $v_{x,t\downarrow}$ visited by $P$. Moreover, identify the ``maximum'' node at each time slot $t\in[T]$ by
\begin{equation*}
...
\begin{equation*}\label{prop_reasn_paths_identify}
	\Phi:\bm{\mathcal{P}}\rightarrow\fromto{0}{m}^T\eqqcolon\bm{\mx},\quad P\mapsto (x_1,\ldots,x_T)
\end{equation*}
is a bijection.
\end{prop}
\begin{proof}
We need to check that $\Phi$ is injective and surjective. 

Let $P,P'$ be reasonable paths with $\Phi(P)=\Phi(P')$. Then $P$ as well as $P'$ must start with the edge $(v_{0,0},v_{x_1,1\downarrow})$. As $P$ and $P'$ are reasonable, their paths between $v_{x_t,t\downarrow}$ and $v_{x_{t+1},t+1\downarrow}$ must coincide for $t\in[T-1]$. Further, the path from $v_{x_T,T}$ to $v_{0,T}$ is unique in our graph. Thus, we have $P=P'$, which shows the injectivity of $\Phi$.

Next, let $(x_1,\ldots,x_T)\in\bm{\mx}$. For any $t\in[T]$, we set
\begin{equation*}
	S'\coloneqq
	\begin{cases}
		(v_{x_s,t\downarrow},v_{x_s,t\uparrow},v_{x_s+1,t\uparrow},\ldots,v_{x_e,t\uparrow}), & \text{if $x_s< x_e$}\\
...
which shows the surjectivity of $\Phi$.
\end{proof}
...
\end{lem}
\begin{proof}
By Proposition~\ref{prop_reasn_paths_identify}, we already have a bijection. Let $P$ be a reasonable path.
