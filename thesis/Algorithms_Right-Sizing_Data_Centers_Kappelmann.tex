...
Moreover, we decreased the number of bad spots\unsure{bad spots? better description?} at time $t$ concerning~\eqref{eq_strict_switching}. Hence, by repeating described process on $\Sigma'$, we obtain a terminating procedure that returns a schedule satisfying the conditions.
\end{proof}
...
\end{cor}
\begin{proof}
Let $\Sigma$ be an optimal schedule for $\inp$. Applying Proposition~\ref{prop_strict_switching} to $\Sigma$ yields $\Sigma^*$ satisfying~\eqref{eq_strict_switching} with $\costs(\Sigma^*)\le\costs(\Sigma)$. The claim follows.
\end{proof}
...
Given $x_t\in\mathbb{N}$ active servers in time slot $t$, an arrival rate $\lambda_t\in[0,x_t]$, and a convex cost function $f$, a most cost-efficient and feasible scheduling strategy is to assign each active server a load of $\lambda_t/x_t$.\unsure{This is sounds awkwardly formulated, doesn't it?}
\end{prop}
\begin{proof}
...
After that, we refine our initial approach to derive an improved algorithm with pseudo-linear complexity $\Theta(Tm)$.\unsure{Sounds strange\ldots}
...
\begin{equation*}
	\costs(P)\stackrel{\eqref{eq_v0_VT1_path_costs}}{=}\underbrace{\costs(0,x_1,\lambda_1)}_{=\costs(x_0,x_1,\lambda_1)}+\sum\limits_{t=2}^{T}\costs(x_{t-1},x_{t},\lambda_t)=\sum\limits_{t=1}^{T}\costs(x_{t-1},x_{t},\lambda_t)\stackrel{\eqref{eq_even_sharing_schedule_costs}}{=}\costs(\mx)
\end{equation*}
\end{itemize}
...
%\begin{algorithm}[H]
    ...
    %\begin{algorithmic}[1]
    %\Function{optimal\_offline}{$m,T,\Lambda,\beta,f$}
	%\Blet{$C[T+1,m]$ and $p[T+1,m]$}{new arrays}\Comment{Costs and parents}
	%\For{$x \gets 0 \textrm{ to } m$}\Comment{Initialisation}
		%\Let{$C[1,x]$}{$c(0,x,\lambda_1)$}
		%\Let{$p[1,x]$}{$0$}
	%\EndFor
	%\For{$t \gets 2 \textrm{ to } T$}\Comment{Iterative calculation of costs and parents}
		%\For{$x' \gets 0 \textrm{ to } m$}
			%\Let{$opt$}{$\infty$}
			%\For{$x \gets 0 \textrm{ to } m$}
				%\Let{$C[t,x']$}{$C[t-1,x]+c(x,x',\lambda_t)$}
				%\If{$C[t,x']<opt$}
					%\Let{$opt$}{$C[t,x']$}
					%\Let{$p[t,x']$}{$x$}
				%\EndIf
		...
		%\EndFor
	%\EndFor
	%\Let{$C[T+1,0]$}{$\min\bigl\{C[T,x]\mid x\in\fromto{0}{m}\bigr\}$} \Comment{Find minimum at last time slot}
	%\Let{$p[T+1,0]$}{$\argmin\limits_{x\in\fromto{0}{m}}\bigl\{C[T,x]\bigr\}$}
	%\State \Return{$C$ and $p$}
  %\EndFunction
  %\end{algorithmic}
...
\end{equation*}
A schedule corresponding to $P$ would use one active server in its first time slot, then power down this server, and subsequently turn on two servers to process the next arrival rate. This seems somewhat unreasonable. We could just keep the initial server on to save switching costs. In fact, this behaviour cannot even be represented using our schedule notation $\mx$ and optimisation condition~\eqref{eq_even_sharing_schedule_costs}. We can, however, modify $P$ to represent a more reasonable sequence, that is we set
\begin{equation*}
...
A path $P$ from $v_{0,0}$ to $v_{0,T+1}$ is called \textit{reasonable} if for any time slot $t\in[T]$ the path does not shut down and power on servers simultaneously at $t$. More formally, $P$ must satisfy the formula
\begin{equation}
...
\begin{prop}
Any given path $P$ from $v_{0,0}$ to $v_{0,T+1}$ corresponds to a reasonable path $P'$ with $\costs(P')\le\costs(P)$.
\end{prop}
\begin{proof}
...
\end{align*}
Next, consider the subpath $S=(v_{x_s,t\downarrow},\ldots,v_{x_e,t\uparrow})$ of $P$. Note that the subpath uses all edges from $E_\downarrow^t$ as well as from $E_\uparrow^t$.
...
\begin{equation*}
	...
	\end{cases}
\end{equation*}
	In each case, $S'$ uses edges from at most one of the sets $E_\downarrow^t$ and $E_\uparrow^t$. Thus, by replacing the subpath $S$ of $P$ with $S'$, we obtain a new schedule $P'$ that satisfies~\eqref{eq_reasn_path} at time $t$. The paths $P$ and $P'$ coincide in their costs before visiting $v_{x_s,t\downarrow}$ and after visiting $v_{x_e,t\uparrow}$; 
however, their switching costs differ in that there are less switching costs $\beta$ at time $t$ using $P'$. As we assume $\beta\ge0$, we conclude $\costs(P')\le \costs(P)$
...
\end{proof}
\begin{prop}
Any given schedule $\mx$ for $\inp$ corresponds to a path $P$ from $v_{0,0}$ to $v_{0,T+1}$ with $\costs(\mx)=\costs(P)$.
\end{prop}
\begin{proof}
Let $\mx=(x_1,\ldots,x_T)$ be a schedule for $\inp$. 
\end{proof}
