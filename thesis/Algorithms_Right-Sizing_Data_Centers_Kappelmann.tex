...
\end{equation}
Note that the costs of such a path directly correspond to those of a schedule $\mx$ (see~\eqref{eq_mx_schedule_total_costs}).
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2cm,thick,node/.style={minimum size=1.2cm,circle,draw}]
  ...
  \path[every node/.style={font=\sffamily\small}]
	...
	edge node[label={[xshift=0.5cm, yshift=-0.8cm]$\costs(0,2,3)$}] {} (5)
	edge node[label={[xshift=0.28cm, yshift=-0.3cm]$\costs(0,1,3)$}] {} (3)
...
\begin{lem}
Let $\bm{\mx}$ be the set of all schedules $\mx$ for $\inp$, and let $\bm{\mathcal{P}}$ the set of all reasonable paths.
Then the map
...
As $P$ is reasonable, we have $E_\downarrow^t=\emptyset \lor E_\uparrow^t=\emptyset$ for any time slot $t$. If $E_\uparrow^t$ is empty, the costs between the nodes $v_{x,t-1\downarrow}$ and $v_{x,t\downarrow}$ are given by $\opcosts(x_{t-1},x_t)$. If $E_\uparrow^t$ is non-empty, the costs are given by $\beta(x_t-x_{t-1})+\opcosts(x_{t-1},x_t)$. The costs of $P$ can thus be calculated by 
\begin{align*}
\costs(P)&=c(0,x_1,\lambda_1)+\sum\limits_{t=2}^{T}\bigl(\beta\max\{0,x_t-x_{t-1}\}+\opcosts(x_{t},\lambda_t)\bigr)\\
	&\stackrel{\eqref{eq_mx_schedule_sw_costs}}{=}c(0,x_1,\lambda_1)+\sum\limits_{t=2}^{T}\bigl(\swcosts(x_{t-1},x_t)+\opcosts(x_{t},\lambda_t)\bigr)\\
