...
First, we reduce our problem specified by $\inp$ to a shortest path problem of a level structured graph $G$. We then use a dynamic programming approach to find a shortest path in $G$ and thereby an optimal schedule for $\inp$ in pseudo-polynomial time $\mathcal{O}(Tm^2)$.
After that, we refine our initial approach to derive an improved algorithm with pseudo-linear complexity $\mathcal{O}(Tm)$.\unsure{Sounds strange\ldots}
...
\textproc{shortest\_paths} uses a dynamic programming approach similar to the well-known Bellman-Ford algorithm. It returns the shortest path to every vertex as well as the predecessor of every vertex in respect to its shortest path.
...
\textproc{shortest\_paths} needs $\mathcal{O}(m)$ iterations for its initialisation, $\mathcal{O}(Tm^2)$ steps for the iterative calculation process, and $\mathcal{O}(m)$ steps for its final minimisation search. In addition, \textproc{extract\_schedule} needs $\mathcal{O}(T)$ iterations for its schedule retrieval. Thus, we receive a runtime of 
\begin{equation*}
...
Our memory demand is defined by the size of the arrays $C$ and $P$. As both arrays are of size $\mathcal{O}(Tm)$, we receive a memory complexity of $\mathcal{O}(Tm)$
...
\begin{tikzpicture}[->,>=stealth',auto,node distance=2.2cm,thick,node/.style={minimum size=1.5cm,circle,draw}]
  \node[node] (1) {0,0};
  \node[node] (4) [below right=4cm of 1] {0,1,u};
  \node[node] (3) [above =0.5cm of 4] {1,1,u};
  \node[node] (2) [above right=4cm of 1] {m,1,u};
  \node[node] (5) [below =0.5cm of 2] {m-1,1,u};
  \node[node] (7) [right =of 3] {1,1};
  \node[node] (6) [right =of 2] {m,1};
  \node[node] (8) [right =of 4] {0,1};
  \node[node] (9) [right =of 5] {m-1,1};
  \node[node] (11) [right =of 7] {1,2,u};
  \node[node] (10) [right =of 6] {m,2,u};
  \node[node] (12) [right =of 8] {0,2,u};
  \node[node] (13) [right =of 9] {m-1,2,u};
  \node[node] (15) [right =of 11] {1,2};
  \node[node] (14) [right =of 10] {m,2};
  \node[node] (16) [right =of 12] {0,2};
  \node[node] (17) [right =of 13] {m-1,2};
  \node[node] (19) [right =4cm of 15] {1,T};
  \node[node] (18) [right =4cm of 14] {m,T};
  \node[node] (20) [right =4cm of 16] {0,T};
  \node[node] (21) [right =4cm of 17] {m-1,T};
  ...
  \path[every node/.style={font=\sffamily\small}]
    ...
    (16) edge node[right] {$\beta$} (15)

    (21) edge node[right] {$\beta$} (18)
    (20) edge node[right] {$\beta$} (19)

    (18) edge node[right] {$0$} (22)
    ...
    (5) edge node[right] {$0$} ($(5)!.35!(3)$)
    ($(5)!.65!(3)$) edge node[right] {$0$} (3)

    ($(7)!.65!(9)$) edge node[right] {$\beta$} (9)
    (7) edge node[right] {$\beta$} ($(7)!.35!(9)$)
 
    (13) edge node[right] {$0$} ($(13)!.35!(11)$)
    ($(13)!.65!(11)$) edge node[right] {$0$} (11)

    ($(15)!.65!(17)$) edge node[right] {$\beta$} (17)
    (15) edge node[right] {$\beta$} ($(15)!.35!(17)$)


    (14) edge node[label={[xshift=0.2cm, yshift=-0.26cm]$\opcosts(m,\lambda_3)$}] {} ($(14)!.4!(18)$)
    ...
    ($(16)!.6!(20)$) edge node[above] {$0$} (20)
    ($(17)!.6!(21)$) edge node[above] {$0$} (21)

    ($(19)!.65!(21)$) edge node[right] {$\beta$} (21)
    (19) edge node[right] {$\beta$} ($(19)!.35!(21)$);
\end{tikzpicture}
}
\caption{Graph for a pseudo-linear, optimal offline algorithm}
\end{figure}
