...
Using above definitions, we can define the input of our model by $\mathcal{I}\coloneqq(m,T,\Lambda,\beta,f)$ where $\Lambda=(\lambda_1,\ldots,\lambda_T)$ is the sequence of arrival rates. Naturally, given an input $\mathcal{I}$, we want to schedule our servers in such a way that we minimise the sum of incurred costs while warranting that we are processing the given loads in time. For this, consider the two sequences 
\begin{align*}
	\mx&\coloneqq(x_1,\ldots,x_T)\in\fromto{0}{m}^T\\
	\mathcal{L}&\coloneqq(\lambda_{1,1},\lambda_{2,1},\ldots,\lambda_{m,1},\lambda_{1,2},\ldots,\lambda_{m,T})\in[0,1]^{m*T}
\end{align*}
where $x_t$ denotes the number of active servers at time $t$ and $\lambda_{i,t}$ the load assigned to server $i$ at time $t$ (as described in section~\ref{sec_model_descr}).
\begin{align*}
	\mathcal{S}^*&=(s_{1,1},s_{2,1},\ldots,s_{m,1},s_{1,2},\ldots,s_{m,T})\in\{0,1\}^{m*T}\\ 
	\mathcal{L}^*&=(\lambda_{1,1},\lambda_{2,1},\ldots,\lambda_{m,1},\lambda_{1,2},\ldots,\lambda_{m,T})\in[0,1]^{m*T}
\end{align*}
where $\mathcal{S}$ is the series of power states and $\mathcal{L}$ is the series of assigned loads for our servers subject to
\begin{align*}
	(\mathcal{S}^*,\mathcal{L}^*)=\argmin\limits_{(\mathcal{S},\mathcal{L})}
	\{\mid\sum\limits_{t=1}^{T}\lambda_t=\sum\limits_{i=1}^{m}\sum\limits_{t=1}^{T}\lambda_{i,t}*s_{i,t}\}
\end{align*}

\subsection{Overview}
Below, we give an overview of just given definitions and conventions:
\scriptsize
\begin{multicols}{2}
\begin{itemize}
	...
	\item $\lambda_{i,t}$\ldots assigned arrival rate for server $i$ at time $t$
	...
	\item $\mx\coloneqq(x_1,\ldots,x_T)$\ldots sequence of active servers
