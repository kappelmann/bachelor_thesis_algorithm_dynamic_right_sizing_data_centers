...
Moreover, we decreased the amount of bad spots\unsure{bad spots? better description?} at time $t$ concerning~\eqref{eq_strict_switching}. Hence, by repeating described process on $\Sigma'$, we obtain a terminating procedure that returns a schedule satisfying the conditions.
\end{proof}
...
A path $P$ from $v_{0,0}$ to $v_{0,T+1}$ is called \textit{reasonable} if for any time slot $t\in[T]$ the path does not power off and power on servers simultaneously at $t$. More formally, $P$ must satisfy the formula
\begin{equation}
...
\end{prop}
\begin{proof}
Let $P$ be a path from $v_{0,0}$ to $v_{0,T+1}$. Further, let $E_\downarrow^t$ and $E_\uparrow^t$ be its sets of edges for $t\in[T]$ as defined in Definition~\ref{defn_reasn_paths}. We give a procedure that repeatedly modifies $P$ such that it satisfies~\eqref{eq_reasn_path} and reduces or retains its costs. 
\end{proof}
\begin{prop}
