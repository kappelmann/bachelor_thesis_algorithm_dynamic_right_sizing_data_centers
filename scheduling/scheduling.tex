...
\usepackage[english]{babel} %% Verwende Umlaute direkt
...
\subsection{A memory optimized algorithm}
TODO

\section{A $3/2$-approximation algorithm for monotonically increasing convex f}
We consider a modification of the problem discussed in chapter~\ref{sec:opt}. Assuming that f is convex and monotonically increasing, we can modify our algorithm to obtain a polynomial time $3/2$-approximation algorithm.

\subsection{Graph for a $3/2$-optimal schedule}
...
First, let $b\coloneqq\lceil\log_2(m)\rceil$. We add vertices $(t,0)$ and $(t,2^i),\forall t\in[T-1],i\in\bigl\{0,\ldots,\lceil\log_2(m)\rceil\bigr\}$.
...
\end{tikzpicture}
\caption{Graph for $3/2$-approximation algorithm}
\end{figure}
\begin{prop}
	A shortest path from $(0,0)$ to $(T,0)$ in the constructed graph delivers a $3/2$-optimal schedule.
\end{prop}
\begin{proof}
	Assume we have an optimal schedule identified by $\mathcal{X}=(x_0,\ldots,x_T)$.
\end{proof}
