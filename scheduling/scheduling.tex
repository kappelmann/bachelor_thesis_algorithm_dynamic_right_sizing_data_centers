...
\end{figure}
	For this reason, we now only have to consider the fraction of costs between $\mx'$ and $\mx$ between every time step $t_{i-1}$ and $t_i$. For this set
	\begin{equation*}
		costs(\mx,t,t')\coloneqq\beta\max\{0,x_{t'}-x_t\}+x_{t'}f(\lambda_{t'}/x_{t'})\label{eq:costs_t_chi_prime}
	\end{equation*}
	and then consider the fraction of costs
	\begin{equation}
		\frac{costs(\mx',t_{i-1},t_i)}{costs(\mx,t_{i-1},t_i)}\label{eq:frac_chi_prime}
	\end{equation}
	For $x_{t_i}=0$ it follows that $costs(\mx',t_{i-1},t_i)=costs(\mx,t_{i-1},t_i)=0$. Hence, we can restrict ourselves to $0<t_i<T$ with $x_{t_i}\neq 0$.
	The costs incurred by $\mx'$ are given by
	\begin{alignat}{3}
		&&&&&costs(\mx',t_{i-1},t_i)\nonumber\\
		&&&=\quad&&\beta\max\{0,x'_{t_i}-x'_{t_{i-1}}\}+x'_{t_i}f(\lambda_{t_i}/x'_{t_i})\nonumber\\
		&&\text(\ref{def:chi_prime})\quad&\le&&\beta\max\{0,x'_{t_i}-x'_{t_{i-1}}\}+4x_{t_i}f(\lambda_{t_i}/x'_{t_i})\nonumber\\
