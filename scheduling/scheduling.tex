...
$ $
\begin{enumerate}
	\item Assume we have an optimal schedule identified by $\mx=(x_0,\ldots,x_T)$. Let $(0=t_0,t_1,\ldots,t_n=0)$ be the sequence of times with $t_i<t_{i+i},\forall 0\le i< n$ where $x_{t_i}=2^{k_i},k_i\in\mathbb{N_0}\forall 0<i<n$For $0<t<T$ we inductively set:
	%\begin{equation}
		%%x'_0=0,\qquad x'_{t+1}=2^{\lfloor\log_2(x'_t+2(x_{t+1}-x_t))\rfloor} \label{def:xprime}
		%x'_0\coloneqq 0,\qquad
		%x'_{t+1}\coloneqq 
		%\begin{cases}
			%\min\bigl\{2^{\lfloor \log_2(2x_{t+1})\rfloor},2^b\bigr\}, & \text{if $0<x_t\le x_{t+1}$}\\
			%x'_t, & \text{if $0<x_{t+1}<x_t$ and $x'_{t}\le 4x_{t+1}$}\\
			 %2^{\lceil \log_2(2x_{t+1})\rceil}, & \text{if $0<x_{t+1}<x_t$ and $x'_{t}>4x_{t+1}$}\\
%\			   0, & \text{otherwise}
		%\end{cases} \label{def:xprime}
	%\end{equation}
		%Then let $\mx'\coloneqq(x'_0,\ldots,x'_T)$ be the modified sequence of active servers. Notice that \makebox{$x_t\le x'_t\le 4x_t$} holds as $x'_t$ is at most the smallest power of two larger than $2x_t$ and therefore $\mx'$ is feasible. We can construct a feasible path in our graph from $\mathcal{X'}$ as follows:\\
	%$\forall t\in[T]$ set $e_t\coloneqq\Bigl(\bigl(t-1,\mx'(t-1)\bigr),\bigl(t,\mx'(t)\bigr)\Bigr)$. Then set $P\coloneqq(e_1,\ldots,e_{T})$. By the definition of the edges' weights it follows $costs(\mx')=costs(P)$.\\
		%Next, lets consider the fraction of costs between $\mx'$ and $\mx$ at every time step $t$ to $t+1$ for $0\le t\le T-1$:
